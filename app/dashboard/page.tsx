...
export default function Dashboard(){
  ...
  return <main className="dash"><header><a className="brand" href="/"><span className="brandMark"><i/><i/><i/></span><span>فتّاشة</span></a><a className="secondary" href="/">بحث جديد</a></header>
    ...
    <section className="dashSection"><h2>سجل البحث والملكية</h2>{loading?<div className="empty">جارٍ تحميل السجل…</div>:history.length?<div className="historyTable">{history.map(item=><article key={item.id}><div className="recordIcon">⌁</div><div><b>{item.filename}</b><small dir="ltr">ATH-{item.id.slice(0,8).toUpperCase()}</small></div><code dir="ltr">{item.sha256.slice(0,18)}…</code><span>{item.availableSources} تطابقات · {item.searchedSources} مصدر آلي</span><time>{new Intl.DateTimeFormat("ar-SA",{dateStyle:"medium",timeStyle:"short"}).format(new Date(item.createdAt))}</time></article>)}</div>:<div className="empty">لا توجد عمليات بحث محفوظة بعد.</div>}</section>
