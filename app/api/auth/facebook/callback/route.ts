...
export async function GET(request:Request){
  ...
  const profileResponse=await fetch(`https://graph.facebook.com/v23.0/me?fields=id,name&access_token=${encodeURIComponent(token.access_token)}`,{cache:"no-store"});const profile=await profileResponse.json() as {id?:string;name?:string};if(!profile.id||!profile.name)return Response.redirect(new URL("/?facebook=failed",request.url));
  const now=new Date();await getDb().insert(facebookUsers).values({facebookId:profile.id,name:profile.name,userId:visitor,createdAt:now,lastLoginAt:now}).onConflictDoUpdate({target:facebookUsers.facebookId,set:{name:profile.name,userId:visitor,lastLoginAt:now}});
